...
MeshStorage::MeshStorage(ResourceAllocator& allocator_in, ResourceUploadQueue& queue_in) :
    ...
    vertex_position_buffer = allocator_in.create_buffer("Vertex position buffer",
                                                        max_num_vertices * sizeof(VertexPosition),
                                                        BufferUsage::VertexBuffer);
    vertex_data_buffer = allocator_in.create_buffer("Vertex data buffer",
                                                    max_num_vertices * sizeof(StandardVertexData),
                                                    BufferUsage::VertexBuffer);
    index_buffer = allocator_in.create_buffer("Index buffer", max_num_indices * sizeof(uint32_t),
...
tl::optional<Mesh> MeshStorage::add_mesh(std::span<const StandardVertex> vertices, std::span<const uint32_t> indices) {
    auto mesh = Mesh{};
    ...
    for (const auto& vertex: vertices) {
        ...
        data.push_back(StandardVertexData{
    ...
    upload_queue->upload_to_buffer<VertexPosition>(vertex_position_buffer, positions,
                                                   static_cast<uint32_t>(mesh.first_vertex * sizeof(VertexPosition)));
    upload_queue->upload_to_buffer<StandardVertexData>(vertex_data_buffer, data,
...
void MeshStorage::free_mesh(Mesh mesh) {
    vmaVirtualFree(vertex_block, mesh.vertex_allocation);
