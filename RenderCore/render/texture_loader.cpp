...
tl::optional<TextureHandle> TextureLoader::load_texture_ktx(const std::filesystem::path& filepath, const TextureType type) {
    return SystemInterface::get()
           .load_file(filepath)
           .and_then(
               [&](const std::vector<uint8_t>& data) -> tl::optional<TextureHandle> {
...
tl::optional<TextureHandle> TextureLoader::load_texture_png(const std::filesystem::path& filepath, TextureType type) {
    return SystemInterface::get()
           .load_file(filepath)
           .and_then(
               [&](const std::vector<uint8_t>& data) -> tl::optional<LoadedTexture> {
           ...
           .and_then(
               [&](const LoadedTexture& texture) -> tl::optional<TextureHandle> {
                   const auto format = [&]() {
                       switch (type) {
