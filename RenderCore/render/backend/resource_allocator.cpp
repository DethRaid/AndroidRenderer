...
TextureHandle ResourceAllocator::create_volume_texture(const std::string& name, VkFormat format, glm::uvec3 resolution,
...
BufferHandle ResourceAllocator::create_buffer(const std::string& name, size_t size, BufferUsage usage) {
    ...
    Buffer buffer;
    auto result = vmaCreateBuffer(vma, &create_info, &vma_create_info, &buffer.buffer, &buffer.allocation,
